...
try:
    ...
    def usr_login():
        ...
        if plain == 'login successfully':
            while True:
                patient_id = tkSimpleDialog.askstring('Python Tkinter','patient ID')

                en_patient_id = f.encrypt(patient_id)
                sock.send(en_patient_id)

                en_id_result = sock.recv(1024)

                id_result = f.decrypt(en_id_result)

                if id_result == 'wrong number! Please enter again.':
                    tkMessageBox.showinfo(title='Error', message= id_result )
                else:
                    break
            patients_data = list()
            while True:
            ...
            btn_update = tk.Button(window_data, text='Update', command=update_data)
            btn_update.place(x=280, y=250)
    ...
    def update_data():
        new_data = tkSimpleDialog.askstring('Python Tkinter', 'Update')
        en_new_data = f.encrypt(new_data)
        sock.send(en_new_data)
